...
const datoCmsAssetFixture = {
  internal: {
    contentDigest: '123'
  },
  entityPayload: {
    attributes: {
      url: 'https://localhost/mocked.svg',
...
describe('general', () => {
  ...
  test('create datocms resolver', async () => {
    createResolvers({
      cache,
      createResolvers: createResolversMock,
      store,
...
describe('gatsby-source-datocms', () => {
  beforeEach(() => {
    createResolversMock.mockClear()
    registeredResolvers.clear()
    reporter.panic.mockClear()
  ...
  test('process DatoCMS Asset', async () => {
    createResolvers({
      cache,
      createResolvers: createResolversMock,
      store,
    ...
    const result = await resolverData.svg.resolve(datoCmsAssetFixture)
  ...
  test('skip non-svg DatoCMS assets', async () => {
    createResolvers({
      cache,
      createResolvers: createResolversMock,
      store,
    ...
    const noSvgFixture = JSON.parse(JSON.stringify(datoCmsAssetFixture))
    noSvgFixture.entityPayload.attributes.mime_type = 'no/svg'
    const result = await resolverData.svg.resolve(noSvgFixture)
  ...
  test('skip empty DatoCMS assets', async () => {
    createResolvers({
      cache,
      createResolvers: createResolversMock,
      store,
    ...
    const noFileFixture = JSON.parse(JSON.stringify(datoCmsAssetFixture))
    delete noFileFixture.entityPayload.attributes.url
    const result = await resolverData.svg.resolve(noFileFixture)
